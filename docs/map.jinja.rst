...
We will see a quick setup to configure the ``tool_firefox`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_firefox`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_firefox`` formulas under ``/srv/salt/tool_firefox/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_firefox/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_firefox/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_firefox/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_firefox/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_firefox`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_firefox`` formulas under ``/srv/salt/tool_firefox/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_firefox/parameters/roles
...
We create a configuration for the role ``tool_firefox/server`` in ``/srv/salt/tool_firefox/parameters/roles/tool_firefox/server.yaml``:
...
We create another configuration for the role ``tool_firefox/client`` in ``/srv/salt/tool_firefox/parameters/roles/tool_firefox/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_firefox/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_firefox/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_firefox/parameters/dns:domain/  '/srv/salt/tool_firefox/parameters/dns!domain/'
